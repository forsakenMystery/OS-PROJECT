struct table_of_contentss {
	int freq; //frequency
	int referred_page; //page
};


int mfu(int pages[], int frames, int size_of_page) {
	struct table_of_contentss page[size_of_page];
	struct table_of_contentss page_table[frames];
	int i,j,k,w;
	...
	for(i=0;i<size_of_page;i++) {
		...
		if((frame_count < frames) && (flag == 0)) {
			...
			int maximum= page_table[0].freq;
			int eq_arr_count=0;
			for (k=0;k<frames;k++) {
				if(page_table[k].freq > maximum) {
					maximum= page_table[k].freq;
					j=k;
				}
			}
			for (k=0;k<frames;k++) {
			...
			if (eq_arr_count > 1) {//if eq_arr_count > 1
				...
				while(k < eq_arr_count)	{
					for(index=0; index<frames; index++) {
						if(page_table[index].freq == maximum){
							eq_arr[k++]=index;//initialise this new array with 'index' values that are equal to maximum
					}
				}
			}
